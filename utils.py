...
def write_simple_1D_function_summary(
    ...
    pred_func = to_numpy(
        model_output["model_out"]["output"].squeeze()
    )  # B, Samples, DimOut
    ...
    freqs = np.fft.fftshift(
        np.fft.fftfreq(oversample * len(val_coords), 1 / len(val_coords))
    )
    ...
    plt.plot(
        freqs, np.fft.fftshift(np.abs(np.fft.fft(val_gt_func))), label="GT", linewidth=2
    )
...
def write_multiscale_image_summary(
    image_resolution,
    train_dataset,
    model,
    model_input,
    ...
    pred_img = [
        dataio.lin2img(out, image_resolution).clamp(0, 1)
        for out in model_output["model_out"]["output"]
    ]
    ...
    if isinstance(model, modules.MultiscaleBACON):
        ...
        spectrums = [
            (torch.clamp(abs(torch.norm(spectrum, dim=1, keepdim=True)), 0, 1))
            ** (1 / 2)
        ...
        if write_images:
            ...
            for idx, im in enumerate(all_imgs):
                ...
                skimage.io.imsave(
                    os.path.join(logdir, f"im_{idx}_{total_steps:04d}.png"), im
                )
            for idx, im in enumerate(spectrums):
                ...
                im = (im * 255).detach().cpu().numpy().astype(np.uint8)
                skimage.io.imsave(
                    os.path.join(logdir, f"spectrum_{idx}_{total_steps:04d}.png"), im
                )
...
def write_multiscale_radiance_summary(
    models,
    train_dataloader,
    val_dataloader,
    loss_fn,
    ...
    with torch.no_grad():
        out_dict = {
            key: process_batch_in_chunks(in_dict, model, chunk_size, progress=pbar)
    ...
    if isinstance(gt_dict["pixel_samples"], list):
        gt_view = [
            v[0:num_views_to_disp_at_training, :, :, :3].detach().cpu()
            for v in gt_dict["pixel_samples"]
        ]
        view_shape = gt_view[0].shape
    else:
        gt_view = (
            gt_dict["pixel_samples"][0:num_views_to_disp_at_training, :, :, :3]
            .detach()
            .cpu()
        )  # Views,H,W,C
    ...
    pred_sigmas = [
        pred[..., -1:] for pred in out_dict["combined"]["model_out"]["output"]
    ]
    pred_rgbs = [pred[..., :-1] for pred in out_dict["combined"]["model_out"]["output"]]

    if isinstance(out_dict["combined"]["model_in"]["t_intervals"], list):
        t_intervals = [
            t_interval for t_interval in out_dict["combined"]["model_in"]["t_intervals"]
        ]
    else:
    ...
    pred_view = make_grid(
        torch.cat(pred_views, dim=0), scale_each=False, normalize=False
    )

    if isinstance(gt_view, list):
        gt_view = make_grid(
            torch.cat(gt_view, dim=0), scale_each=False, normalize=False
        )

    if pred_view.shape[1] < 512:
        scale = 512 // pred_view.shape[1]
        pred_view = torch.nn.functional.interpolate(
            pred_view.unsqueeze(0), scale_factor=scale, mode="nearest"
        )
        pred_view = pred_view.squeeze(0)
        gt_view = torch.nn.functional.interpolate(
            gt_view.unsqueeze(0), scale_factor=scale, mode="nearest"
        )
    ...
    with torch.no_grad():
        ...
        if hierarchical_sampling:
            ...
            for i in range(4):
                ...
                pred_z_weights.append(
                    forward_models.compute_transmittance_weights(
                        sigma_tmp, t_interval_tmp
                    )
                )
    ...
    gt_dict_reshaped["pixel_samples"] = [
        g.view(1, -1, 3) for g in gt_dict_reshaped["pixel_samples"]
    ]
    ...
    for idx, (sigma, rgb) in enumerate(zip(pred_sigmas, pred_rgbs)):
        ...
        pred_depth = forward_models.compute_tomo_depth(
            pred_z_weights[idx], meta_dict["zs"]
        )
        pred_disp = forward_models.compute_disp_from_depth(
            pred_depth, pred_z_weights[idx]
        )

        pred_view = pred_pixels.view(view_shape).detach().cpu().permute(2, 0, 1)
        pred_view = torch.clamp(pred_view, 0, 1)
        pred_disp_view = (
            pred_disp.view(*view_shape[:2], 1).detach().cpu().permute(2, 0, 1)
        )
    ...
    pred_disp_view = make_grid(
        torch.cat(pred_disps, dim=0), scale_each=False, normalize=False
    )
    ...
    if gt_view.shape[1] < 512:
        ...
        pred_disp_view = torch.nn.functional.interpolate(
            pred_disp_view.unsqueeze(0), scale_factor=scale, mode="nearest"
        )
        pred_view = pred_view.squeeze(0)
...
def write_radiance_summary(
    ...
    writer.add_video(
        "train: GT",
        gt_view.permute(0, 3, 1, 2)[:, None, :, :, :],
        global_step=total_steps,
    )
    writer.add_video(
        "train: Pred",
        pred_view.permute(0, 3, 1, 2)[:, None, :, :, :],
        global_step=total_steps,
    )
    ...
    for n in range(
        num_samples
    ):  # we run a for loop of num_samples instead of a batch to use less cuda mem
        in_dict, meta_dict, gt_dict = next(iter(val_dataloader))
        ...
        gt_dict_reshaped = gt_dict.copy()
        gt_dict_reshaped["pixel_samples"] = gt_dict_reshaped["pixel_samples"].view(
            1, -1, 3
        )
        losses = loss_fn(out_dict, gt_dict_reshaped)
        ...
        pred_view = torch.clamp(pred_view, 0, 1)
        pred_disp_view = (
            pred_disp.view(gt_view[:, :, 0:1].shape).detach().cpu().permute(2, 0, 1)
        )
        gt_view = gt_view.permute(2, 0, 1)
...
def write_sdf_slice(model, writer, total_steps, prefix="train_", is_multi=False):
    ...
    with torch.no_grad():
        ...
        fig, axs = plt.subplots(
            1, len(all_sdf_values), figsize=(2.75 * len(all_sdf_values), 2.75), dpi=100
        )
...
def write_multiscale_sdf_summary(
...
def plot_samples(out_dict, num_rays_to_visu=10, xlim=(0, 6)):
    ...
    idcs = (
        torch.arange(0, num_rays_to_visu).reshape(-1, 1).repeat(1, num_samples).float()
    )
    ...
    t_transformed = (
        torch.cumsum(t_intervals, dim=-1).cpu().detach()
    )  # we could have t but it requires many more changes
    num_rays = t_transformed.shape[0]
    ts = t_transformed[num_rays // 2 : num_rays // 2 + num_rays_to_visu, :-1]
    num_samples = ts.shape[1]
    idcs = (
...
def process_batch_in_chunks(in_dict, model, max_chunk_size=1024, progress=None):
    ...
    for key in in_dict:
        ...
        chunks = torch.split(
            in_dict[key].view(-1, num_samples_per_rays, num_dims), max_chunk_size
        )
        in_chunked.append(chunks)

    list_chunked_batched_in = [
        {k: v for k, v in zip(in_dict.keys(), curr_chunks)}
        for curr_chunks in zip(*in_chunked)
    ]
    del in_chunked
        ...
        for key in tmp["model_out"]:
            ...
            if isinstance(tmp["model_out"][key], list):
                for idx, elem in enumerate(tmp["model_out"][key]):
                    ...
                    list_chunked_batched_out_out.setdefault(
                        key, [[] for _ in range(len(tmp["model_out"][key]))]
                    )[idx].append(out_)
            else:
