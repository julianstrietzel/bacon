...
def export_model(
    ...
    render_coords = torch.stack((x.flatten(), y.flatten(), z.flatten()), dim=-1).to(
        device
    )
        ...
        for scene, N in zip(scenes, Ns):
            ...
            with open("/tmp/render.cxc", "w") as f:
                for i in range(len(sdfs_ft)):
                    ...
                    f.write(
                        "view matrix camera 0.91721,0.10246,-0.38499,-533.37,-0.010261,0.97212,0.23427,631.78,0.39826,-0.21092,0.89269,1870.6\n"
                    )
                    f.write("view\n")
                    f.write(
                        f"save /tmp/{method}_{scene}_{i + 1}.png width 512 height 512 transparentBackground false\n"
                    )
                    f.write("close #1\n")
                f.write("exit\n")
            ...
            for idx in range(len(sdfs_ft)):
                ...
                os.system(
                    f"./magicwand 1,1 -t 9 -r outside -m overlay -o 0 /tmp/{fname} {fname}"
                )
