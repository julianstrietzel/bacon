...
class BACON(MFNBase):
    ...
    def forward_mfn(self, input_dict):
        if "coords" in input_dict:
            ...
            if self.in_size > 3:
                coords = torch.cat(
                    (input_dict["ray_samples"], input_dict["ray_orientations"]), dim=-1
                )
            else:
...
class MultiscaleBACON(MFNBase):
    def __init__(
        ...
        self.output_linear = nn.ModuleList(
            [nn.Linear(hidden_size, out_size) for i in range(len(self.filters))]
        )
        self.output_linear.apply(mfn_weights_init)
...
def IntegratedPositionalEncoding(coords, radius, L=8):
    ...
    def integrated_pos_enc(x_coord, min_deg, max_deg):
        ...
        return expected_sin(
            torch.cat([y, y + 0.5 * np.pi], dim=-1), torch.cat([y_var] * 2, dim=-1)
        )[0]
...
class FCBlock(nn.Module):
    ...
    def __init__(
        ...
        if isinstance(nonlinearity, list):
            ...
            self.net = []
            self.net.append(
                nn.Sequential(nn.Linear(in_features, hidden_features[0]), first_nl)
            )
            ...
            if outermost_linear:
                ...
                self.net.append(
                    nn.Sequential(
                        nn.Linear(hidden_features[-1], out_features), copy.deepcopy(nl)
                    )
                )
