...
def train(
    ...
    if use_lr_scheduler:
        ...
        def sampling_scheduler(step, start=0, lr0=1e-4, lrn=1e-4):
            if step > start:
                fine_scale = lr_log_schedule(
                    step - start, num_steps=steps - start, nw=1, lr0=lr0, lrn=lrn
                )
        ...
        log_scheduler = partial(
            lr_log_schedule, num_steps=steps, nw=1, lr0=lr, lrn=1e-4
        )
        ...
        for step in range(steps):
            if not step % steps_til_checkpoint and step:
                torch.save(
                    model.state_dict(),
                    os.path.join(
                        checkpoints_dir, "model_step_%04d.pth" % (step + ckpt_step)
                    ),
                )
                np.savetxt(
                    os.path.join(
            ...
            for loss_name, loss in losses.items():
                ...
                if loss_schedules is not None and loss_name in loss_schedules:
                    writer.add_scalar(
                        loss_name + "_weight", loss_schedules[loss_name](step), step
                    )
            ...
            if not step % steps_til_summary:
                tqdm.write(
                    "Step %d, Total loss %0.6f, iteration time %0.6f"
...
def lr_log_schedule(it, num_steps=1e6, nw=2500, lr0=1e-3, lrn=5e-6, lambdaw=0.01):
    return (
        lambdaw + (1 - lambdaw) * np.sin(np.pi / 2 * np.clip(it / nw, 0, 1))
...
def train_wchunks(
    ...
    optims = {
        key: torch.optim.Adam(lr=1, params=model.parameters())
        for key, model in models.items()
    }
    schedulers = {
        key: torch.optim.lr_scheduler.LambdaLR(optim, lr_lambda=lr_log_schedule)
    ...
    with tqdm(total=num_steps) as pbar:
        ...
        for step in range(start_step, num_steps):
            if not step % steps_til_checkpoint and step:
                for key, model in models.items():
                    torch.save(
                        model.state_dict(),
                        os.path.join(
                            checkpoints_dir, "model_" + key + "_step_%04d.pth" % step
                        ),
                    )
                    np.savetxt(
                        os.path.join(
                            checkpoints_dir, "train_losses_step_%04d.txt" % step
                        ),
                        np.array(train_losses),
                    )
                for key, optim in optims.items():
                    torch.save(
                        ...
                        os.path.join(
                            checkpoints_dir, "optim_" + key + "_step_%04d.pth" % step
                        ),
                    )
            ...
            if chunked:
                ...
                batch_avged_tot_loss = 0.0
                for chunk_idx, (
                    chunked_model_input,
                    chunked_meta,
                    chunked_gt,
                ) in enumerate(
                    zip(list_chunked_model_input, list_chunked_meta, list_chunked_gt)
                ):
                    ...
                    chunk_model_outputs = {
                        key: model(chunked_model_input) for key, model in models.items()
                    }
                    for k in models.keys():
                    ...
                    if hierarchical_sampling:
                        ...
                        chunked_model_input_fine = sample_pdf(
                            chunked_model_input, chunk_model_outputs, idx=0
                        )

                        chunk_model_importance_outputs = {
                            key: model(chunked_model_input_fine)
                            for key, model in models.items()
                        }

                        reg_lambda = reg_schedule(step)
                        losses_importance = loss_fn(
                            chunk_model_importance_outputs,
                            chunked_gt,
                            regularize_sigma=True,
                    ...
                    if hierarchical_sampling:
                        for loss_name, loss in losses_importance.items():
                            ...
                            if loss_name + "_importance" in batch_avged_losses:
                                batch_avged_losses[loss_name + "_importance"] += (
                                    single_loss / num_chunks
                                )
                            else:
                                batch_avged_losses.update(
                                    {
                                        loss_name
                                        + "_importance": single_loss / num_chunks
                                    }
                                )
                ...
                model_outputs = {
                    key: model(model_input) for key, model in models.items()
                }
...
def sample_pdf(model_inputs, model_outputs, offset=5e-3, idx=-1):
    """hierarchical sampling code for neural radiance fields"""

    z_vals = model_inputs["t"]
    bins = 0.5 * (z_vals[..., 1:, :] + z_vals[..., :-1, :]).squeeze()
    ...
    ).squeeze()  # batch_pixels, num_bins=samples_per_ray-1)
    cdf = cdf.detach()
    ...
    t_intervals = torch.cat(
        (t_intervals, 1e10 * torch.ones_like(t_intervals[:, 0:1])), dim=-1
    )
