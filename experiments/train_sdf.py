...
def init_model(opt):
    ...
    if opt.ckpt_step:
        ...
        pth_file = "{}/checkpoints/model_step_{}.pth".format(
            opt.root_path, str(opt.ckpt_step).zfill(4)
        )
        model.load_state_dict(torch.load(pth_file))
        print(
            f"Loaded checkpoint from {pth_file}, continue training from step {opt.ckpt_step}"
        )
...
def init_loss(opt):
    ...
    if opt.multiscale:
        ...
        loss_fn = partial(
            loss_functions.multiscale_overfit_sdf, coarse_loss_weight=opt.coarse_weight
        )
    else:
        summary_fn = utils.write_sdf_summary
        loss_fn = partial(
            loss_functions.overfit_sdf, coarse_loss_weight=opt.coarse_weight
        )
