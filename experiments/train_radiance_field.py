...
p.add_argument(
    "--num_steps", type=int, default=1000000, help="Number of iterations to train for."
)
p.add_argument(
    "--steps_til_ckpt",
    type=int,
...
p.add_argument(
    "--supervise_hr",
    action="store_true",
    help="supervise only with high resolution signal",
)
p.add_argument(
    "--use_resized", action="store_true", help="use explicit multiscale supervision"
)
p.add_argument(
    "--reuse_filters",
    action="store_true",
    help="reuse fourier filters for faster training/inference",
)
...
def train(validation=True):
    ...
    if opt.resume is not None:
        ...
        state_dict = torch.load(
            path + "/checkpoints/" + f"model_combined_step_{step:04d}.pth"
        )
        ...
        try:
            resume_checkpoint = {}
            ckpt = torch.load(
                path + "/checkpoints/" + f"optim_combined_step_{step:04d}.pth"
            )
    ...
    if opt.multiscale:
        loss_fn = partial(
            loss_functions.multiscale_radiance_loss, use_resized=opt.use_resized
        )
        summary_fn = partial(
