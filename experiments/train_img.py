...
p.add_argument(
    "--model", default="mfn", choices=["mfn", "mlp"], help="use MFN or standard MLP"
)
...
p.add_argument("--num_steps", type=int, default=5001, help="number of training steps")
p.add_argument("--gpu", type=int, default=0, help="gpu id to use for training")
...
p.add_argument(
    "--use_resized", action="store_true", default=False, help="use multiscale"
)
...
p.add_argument(
    "--img_fn",
    type=str,
    default="../data/lighthouse.png",
    help="path to specific png filename",
)
p.add_argument(
    "--grayscale", action="store_true", default=False, help="if grayscale image"
)
...
def init_dataloader(opt):
    ...
    trn_dataset = dataio.ImageFile(
        opt.img_fn, grayscale=opt.grayscale, resolution=(opt.res, opt.res), url=url
    )

    val_dataset = dataio.ImageFile(
        opt.img_fn,
        grayscale=opt.grayscale,
        resolution=(2 * opt.res, 2 * opt.res),
        url=url,
    )

    trn_dataset = dataio.ImageWrapper(
        trn_dataset,
    ...
    val_dataset = dataio.ImageWrapper(
        val_dataset,
        centered=opt.centered,
        include_end=False,
        multiscale=opt.use_resized,
...
def init_loss(opt, trn_dataset, val_dataset):
    ...
    if opt.multiscale:
        loss_fn = partial(
            loss_functions.multiscale_image_mse, use_resized=opt.use_resized
        )
        summary_fn = partial(
            utils.write_multiscale_image_summary,
            (opt.res, opt.res),
            trn_dataset,
            use_resized=opt.use_resized,
            val_dataset=val_dataset,
        )
    else:
        loss_fn = loss_functions.image_mse
        summary_fn = partial(
            utils.write_image_summary,
            (opt.res, opt.res),
            trn_dataset,
            val_dataset=val_dataset,
        )
