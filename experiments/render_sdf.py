...
def compute_one_scale(model, layer_ind, render_coords, sdf_values, hash_ind):
    ...
    for i in range(int(len(render_coords) / bsize) + 1):
        coords = render_coords[i * bsize : (i + 1) * bsize, :]
        out = model({"coords": coords}, specified_layers=output_layers[layer_ind])[
            "model_out"
        ]
...
def compute_one_scale_adaptive(
    ...
    for i in range(int(len(render_coords) / bsize) + 1):
        coords = render_coords[i * bsize : (i + 1) * bsize, :]
        out = model({"coords": coords}, specified_layers=2, get_feature=True)[
            "model_out"
        ]
        sdf = out[0][0]
        if output_layers[layer_ind] > 2:
            ...
            out = model(
                {"coords": coords_surf},
                specified_layers=output_layers[layer_ind],
                continue_layer=2,
                continue_feature=feature_surf,
            )["model_out"]
            sdf_near = out[0]
...
def export_meshes(adaptive=True):
    if model_type == "ff":
        ...
        ckpts = [
            str(os.path.join("../ff_trained_models", os.fsdecode(file)))
            for file in os.listdir("../ff_trained_models")
        ]
